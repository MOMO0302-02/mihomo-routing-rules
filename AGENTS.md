...
- 公开规则版本：`v2026.08.24.5GoogleAITK`（**23 类、929 条**）。2026-08-24 深夜：①README 分类数过期已修并入校验器门禁（负对照验证）；②补收 Perplexity 上传链 2 条 + `clawhub.ai`；③**每月自动体检**上线（`monthly-health.yml`，每月 1 号跑迁移探测+死域检测+官方名单差距，有发现自动开 Issue，全净静默）；④`geosite_gap.py` 入库，配套 `tools/geosite_rejected.txt` 已否决名单（22 条带理由，月报只报真新增；对官方 AI 名单当前差距 0）。
