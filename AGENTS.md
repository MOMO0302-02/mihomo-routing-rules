...
- 2026-09-03 **精简第一步：删 6 条通用第三方 SaaS**（933 → 927，`ai_custom` 242→236）：
  `segment.io/.com/segmentapis.com`、`intercom.com`、`intercomassets.com`、`launchdarkly.com`。
  这批是 2026-08-17 就记在待办里的「`ai_custom` 收了 22 组通用第三方 SaaS 全域」问题，
  当时判为**需用户决策**；2026-09-03 使用者拍板「分两步，先删最安全的那批」。
  判据：**与 ChatGPT 登录链无关**才删；在登录链或使用链上的一律留着，且把保留理由
  写进 CHANGELOG（`intercom.io` 在使用链、`intercomcdn.com` 留待第二步、
  `oaistatsig.com` 是 OpenAI 自家域名），**防止下一轮有人顺手一起删**。
  ⏳ 第二步（17 条有精确对应的宽后缀）**暂缓**：精确条目是抓包得来的，上游换端点时
  宽后缀能兜住而精确规则会漏，且故障表现是「登录卡住不报错」，极难倒查。
  先跑一段时间确认无异常再议。

- 2026-09-03 **精简第二步：删 3 条宽后缀遥测域名**（927 → 924）：`sentry.io`、
  `datadoghq.com`、`intercomcdn.com`（`openai_login_custom` 有精确到主机名的对应，
  删前逐条核对仍在）。
  📌 **原计划删 17 条，实际只删 3 条** —— 拆开看才发现那 17 条分三类：
  **12 条在两套规则集里一模一样**（认证链：auth0/workos/arkoselabs/funcaptcha/
  challenges.cloudflare/statsig/featuregates/ct.sendgrid），删了对同时启用两套的人
  零变化、却会坑只用 `ai_custom` 的人，与「独立完整」承诺相悖 → **保留**；
  **3 条宽窄有别** → 删；**4 条只有 ai_custom 有**（OpenAI 自家 CDN、oaistatsig、
  intercom.io）→ 保留。
  ⚠️ 同时更正早先一条风险判断：曾说「删了可能让 ChatGPT 登录卡住」，**不成立** ——
  认证链那几条全在「一模一样」那组、本就不删；删的 3 条纯遥测，最坏只是换个出口。
  **教训：一批规则不要当成同质的整体评估风险，先按「有没有精确替代、替代的范围一样不一样」分类。**
