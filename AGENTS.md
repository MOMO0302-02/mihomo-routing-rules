...
- 公开规则版本：`v2026.08.24.7GoogleAITK`（**23 类、928 条**）。用户实测抓到 Datadog 区域遥测域漏网（`browser-intake-us5-datadoghq.com` 落兜底），`ai_custom` 该条后缀改关键词 `browser-intake`，矩阵验证全变体命中；`openai_login_custom` 的精确写法保持不动。新增 `tools/watch_fallback.py` 实时监控兜底漏网主机（需客户端开外部控制；实测用户当前 VPS 配置未开——7892 是代理端口不是控制器）。
