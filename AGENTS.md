...
- 公开规则版本：`v2026.08.25.1GoogleAITK`（**23 类、929 条**）。2026-08-25 修复共性扫描确认的唯一真风险：`tiktok_custom` 加品牌关键词 `capcut`（与 tiktok/douyin/adobe 先例一致，覆盖 capcutapi/capcutcdn 两族的未来区域兄弟域，矩阵验证无误伤），显式后缀保留。**8-24 共性扫描的 11 条至此全部处置完毕**（1 修复、10 甄别为无需动作）。
