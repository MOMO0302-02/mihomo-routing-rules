# Changelog
## v2026.09.03.3ZhipuDirect

- **修补「国内 AI 走直连」的漏网之鱼：`z.ai` 从 `ai_custom` 挪到 `ai_api_direct_custom`**
  （232 / 31，总量仍 924 —— 是搬家不是增删）。
  09-03 那次把智谱的 `bigmodel.cn` / `zhipuai.cn` / `chatglm.cn` 都改成了直连，
  唯独漏掉它的国际域名 `z.ai`（域名不像中国站，扫关键词时没命中），
  结果 `chat.z.ai`、`zcode.z.ai` 仍被 `ai_custom` 拦去走美国出口，
  智谱因此按海外身份接待、只给谷歌登录。
- **实测依据**（用国内 DoH 223.5.5.5 解析）：
  `zcode.z.ai` → `112.194.174.x`，**是中国境内 IP**，绕美国纯属反向绕远；
  `z.ai` / `chat.z.ai` → `8.216.13x.x`，阿里云新加坡国际站，境内直连可达。
- **同批复核未动的两条**（长得像国内厂商、其实是海外服务，保持走代理）：
  `genspark.ai`（美国公司）、`coze.com`（字节海外版，国内版是 `coze.cn`）。
- 📌 教训：判断「是不是国内 AI」不能只看域名后缀，要看**实际解析到哪**。
